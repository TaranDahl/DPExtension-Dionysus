...
{
    [Serializable]
    public partial class BulletExt : Extension<BulletClass>, IDecorative, IDecorativeInterface<IEventDecorator>
    {
        ...
        public static Pointer<BulletClass> CreateBulletWithDecorator<TDecorator>(Pointer<BulletTypeClass> type, Pointer<AbstractClass> target, Pointer<TechnoClass> owner, Pointer<WeaponTypeClass> weapon, DecoratorId id, params object[] parameters) where TDecorator : Decorator
        {
            var bullet = type.Ref.CreateBullet(target, owner, weapon);
            var technoExt = BulletExt.ExtMap.Find(bullet);
            technoExt.CreateDecorator<TDecorator>(id, "", parameters);
            return bullet;
        }
